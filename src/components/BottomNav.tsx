...
import { Home, ListMusic, CloudSun, Plane, ClipboardCheck } from "lucide-react";
...
const items = [
  ...
  { to: "/sjekkliste", label: "Sjekkliste", icon: ClipboardCheck },
] as const;
        ...
        {items.map(({ to, label, icon: Icon }) => {
          ...
          return (
            <li key={to}>
              <Link
                to={to}
                className={`flex flex-col items-center gap-1 px-1 py-2.5 text-[11px] font-semibold uppercase tracking-wide transition-colors ${
